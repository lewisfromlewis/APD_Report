...
APDmortality <- APD %>% select(PatientId, APACHECategory, ANZROD, ICUOutcome) %>% mutate(Mortality = as.numeric(ICUOutcome=="Died")) %>% group_by(APACHECategory) %>%  summarise("Admissions" = n(), "Observed Mortality" = round(mean(Mortality), 3), "ANZROD Predicted Mortality" = round(mean(ANZROD, na.rm=T), 3)) #%>% filter(`Observed Mortality` > 0.001)
...
As a starting point, however, we chose the following filters: 
    1 Worst P:F ratio <150 mmHg / atm OR 
    2 Cardiac failure admission diagnosis AND 
    3 Died in ICU OR 
    4 Transferred to another hospital ICU 
