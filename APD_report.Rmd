...
missed <- APD %>% 
   filter(APACHEDiagnosis %in% c(501:504, 212) &
      ICU_ADM_DTM-HOSP_ADM_DTM>172800)

```

There were there were `r length(missed$SLK)` admissions, of `r length(unique(missed$SLK))` individuals. The predicted mortality in this sub-cohort was `r round(mean(missed$ANZROD), 3)`. The actual mortality was `r round(mean(missed$ICUOutcome=="Died",na.rm=T),3)`.
