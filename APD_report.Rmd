...
```{r fig-margin, fig.margin = TRUE, echo = FALSE, message = FALSE, warning=FALSE}
ANZROD_by_outcome <- kable(APD %>% 
   group_by(ICUOutcome) %>% 
   summarise("Mean ANZROD" = paste(
      round(mean(ANZROD, na.rm=T), 2),
      "+/-", 
      round(sd(ANZROD, na.rm=T), 2), 
      sep = " ")
      ),
   caption = "Algorithmically predicted mortality for each category of ICU outcome"
   )
mar
```
