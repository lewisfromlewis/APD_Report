...
# There is one way to get the data and one only. The content of POST on developer tools in Firefox when using the COMET front end is https://comet.anzics.com.au/api/v1/reports/runreport. Enter the variables COMETUSR and COMETPWD in the console *outside of this script* to avoid breaches.

#Reportdatefrom <- as.Posixct("2021-01-01T00:00:00+11:00")
#Reportdateto <- as.Posixct("2021-01-30T00:00:00+11:00")
#POST(
#    url = "https://comet.anzics.com.au/api/v1/reports/runreport",
#    authenticate(user = COMETUSR, password = COMETPWD),
#    encode = "json",
#    body = list(
#        type = "ANZICS.Domain.Comet.Reporting.ReportDefinition, ANZICS.Domain",
#        careUnits = "57e139c6-cbec-e611-80c8-005056a105bd",
#        icuFromAdmissionDateTime = "Reportdatefrom",
#        icuToAdmissionDateTime = "Reportdateto",
#        report = "1"
#    )
#)
...
APD_source <- "COMET-Extract-APD.csv"
#APD_source <- paste0("./APD_", params$year, ".csv")
#APD_source <- "RDHAPD_2022_08.csv"
...
cols_APD <- cols(
   ...
   AP3score = col_double(),
   AP3ROD = col_double(),
   `ANZROD 2015` = col_double(),
   `ANZROD 2020` = col_double(),
...
# ANZROD IS A PROBLEM, with the changed prediction model and the insisting on reporting it with a space in the variable name, and the failure to capture `ANZROD 2020` for a large number of patients. This is a big mess. So I'm still using ANZROD 2015, and trying to track down where the problems are so we can report adequately.
...
SPICY <- APD %>% filter(INVDayOne=="TRUE" & INV_Hrs>48 & AgeICU>=65 & TreatmentGoals=="Full Active Treatment" & ICUAdmission == "First" & Frailty %in% c("Well", "ManagingWell", "MildlyFrail", "ModeratelyFrail"))
...
```{r Moves.plot, fig.margin = TRUE, fig.cap = "Instantaneous occupancy over the period.", fig.width=3.5, fig.height=3.5, cache=FALSE, message=FALSE, warning=FALSE, echo = FALSE}
Moves_plot + 
...
Some of the patients in this report will not have left hospital; sometimes they will have been sent to another hospital and will still be there, or might have gone home from there and somehow they will not be linked up. 

In this period, `r sum(APD$HospitalOutcome=='OtherAcuteHospital', na.rm=T)` patients were transferred out to another acute hospital. 

## Who were they? 

The age breakdown is in the table below, with graphical representation of the same data in the graph.

```{r demographics}
# Table of age brackets

demographics_decades <- APD %>% 
    select(AgeICU, ElectiveSurg, Indigenous) %>% 
    mutate(AGEquintile = ntile(AgeICU, 5)) %>% 
    mutate(agegp = cut(AgeICU, breaks = c(0,19, 29, 39, 49, 59, 69, 79, 89, 115))) %>% 
    mutate(pctInd = sum(Indigenous==TRUE/sum(AgeICU>=0)))

demographicstable <- APD %>% 
   select(AgeICU, ElectiveSurg, Indigenous) %>% 
   mutate(AGEquintile = ntile(AgeICU, 5)) %>% 
   group_by(Indigenous, AGEquintile) %>% 
   summarise(
      Patients = n())

demographicsbars <- ggplot(demographics_decades %>% group_by(Indigenous)) + 
    geom_bar(aes(x=agegp, fill = Indigenous)) +
    ggtitle("Proportion of each age quintile made up by Indigenous people") +
    xlab("Age brackets in decades") +
    theme(axis.text.x = element_text(angle=45, hjust=1))

```
```{r demographics-table, fig.fullwidth = TRUE, echo = FALSE, message = FALSE, warning=FALSE}
kable(demographicstable)
```

```{r demographics-bars, fig.fullwidth = TRUE, echo = FALSE, message = FALSE, warning=FALSE}
demographicsbars
```

In total, `r round(sum(APD$Indigenous, na.rm = T)/length(APD$Indigenous)*100,0)`% of the admissions were listed as Indigenous; the overall median age was `r median(APD$AgeICU, na.rm=T)`, and for Indigenous patients was `r median(APD$AgeICU[APD$Indigenous==TRUE], na.rm=T)`. 
...
```{r check.exit.block, message = FALSE, fig.margin = TRUE, fig.cap = "Check the exit block numbers make sense.", fig.width=3.5, fig.height=3.5, cache=FALSE, message=FALSE, warning=FALSE, echo = FALSE}
...
SMR <- APD %>% 
    select(AP3ROD, HospitalOutcome) %>% 
    summarise(
      Patients = n(),
      Predicted = round(mean(AP3ROD, na.rm=T), 3), 
