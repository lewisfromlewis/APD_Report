...
APD <- read_csv("C:/Users/lewis/Documents/The_Information/Research_of_me/Observational/Report_APD/COMET-Extract-APD.csv", na = c("", "NA", "NULL"), col_types = cols_APD)
...
IWO <- rep(0, length(Patient_movements$Event)); for(i in seq_along(Patient_movements$Event)){
   if(i == 1)
      {IWO <- Patient_movements$ANZROD[i]}
   else if(is.na(Patient_movements$ANZROD[i]))
      {IWO[i] <- IWO[i-1]}
   else if(Patient_movements$Event[i] == "Admit")
      {IWO[i] = IWO[i-1]+Patient_movements$ANZROD[i]}
   else if(Patient_movements$Event[i] == "Discharge")
   {IWO[i] = IWO[i-1]-Patient_movements$ANZROD[i]} 
}

Patient_movements <- Patient_movements %>% 
   mutate(Occupancy = cum_occ, IntensityWtOcc = IWO)
...
Weight_moves_plot + 
   ...
   geom_line(aes(y = IntensityWtOcc-mean(IntensityWtOcc)+mean(Occupancy)), colour = "red", alpha = 0.5) + 
   geom_smooth(aes(y = IntensityWtOcc-mean(IntensityWtOcc)+mean(Occupancy)), colour = "pink", alpha = 0.5) +
   labs(title = "Intensity Weighted Occupancy",
        y = "Occupancy") 
   
```

```{r fig-margin, fig.margin = TRUE, fig.cap = "Instantateous occupancy over the period.", fig.width=3.5, fig.height=3.5, cache=TRUE, message=FALSE}
Moves_plot + 
   geom_line()
```
...
occupancy_intensity +
   geom_rect(mapping = aes(xmin = ICU_ADM_DTM, 
                           ...
                           fill= ANZROD,
                           alpha = 0.1)) +
   labs(title = "Intensity weighted admission rate",
        legend = "Try",
        x = "Time spent in ICU",
        y = "Admission date",
...
A summary version of this information is to plot occupancy weighted numerically for the admission ANZROD score, as below.

```{r echo = FALSE, message = FALSE, warning=FALSE}
Intensity_weighted_plot <- ggplot(Patient_movements, mapping = aes(x=Time, y=IWO)) + geom_density()
Intensity_weighted_plot
```

## Stratified by ICU outcome 

This plot repeats the above analysis, and codes the admissions by outcome: those who survived are a shade of blue, and those who died are a shade of red. The lowest risk of death among those who actually went on to die against the prediction of the algorithm is displayed in the deepest red to emphasise those patients. 
...
Over this period, the ANZROD predicted risk of death for those who died was `r round(median(APD$ANZROD[APD$ICUOutcome=="Died"], na.rm=T), 3)`, whereas in survivors the median predicted risk of death was `r round(median(APD$ANZROD[APD$ICUOutcome!="Died"], na.rm=T), 3)`. 
