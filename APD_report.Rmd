...
APDmortality <- APD %>% select(PatientId, APACHECategory, ANZROD, ICUOutcome) %>% mutate(Mortality = as.numeric(ICUOutcome=="Died")) %>% group_by(APACHECategory) %>%  summarise("Admissions" = n(), "Observed Mortality" = round(mean(Mortality), 3), "ANZROD Predicted Mortality" = round(mean(ANZROD, na.rm=T), 3)) 
...
The readmission rate is a source of perennial interest. In sympathy with the language of patient journeys, a readmission can be viewed as a backward step. The expectation may be that on discharge from ICU, patients are either a path to wellness or death and that unexpected divagations from that path are unwelcome. In this period, `r APD %>% filter(ReadmissionEpisode >0) %>% select(PatientId) %>% nrow()` patients were readmitted, accounting for `r APD %>% filter(ReadmissionEpisode >0) %>% select(PatientId) %>% n_distinct()` readmissions in total. The mortality rate was `r round(mean(APD$HospitalOutcome[APD$ReadmissionEpisode >0]=="Died", na.rm=T), 3)`, compared to `r round(mean(APD$HospitalOutcome[APD$ReadmissionEpisode ==0]=="Died", na.rm=T), 3)` for first admissions. 
...
The table below gives further detail on the variables with the greatest proportion of missing data. Many of these are simply not useful. Many are extremely useful, but missed for various reasons.

```{r echo=FALSE}
naniar::miss_var_summary(APD, n=20)
```
...
We often only review the predicted risk of death for those who died, because that is the structure of our quality reporting. Over this period, the table below shows the ANZROD-predicted risk of death for those who died was `r round(median(APD$ANZROD[APD$ICUOutcome=="Died"], na.rm=T), 3)`. As a corrective, it's important also to review the predicted risk of death in survivors. Across this period the median predicted risk of death in survivors was `r round(median(APD$ANZROD[APD$ICUOutcome!="Died"], na.rm=T), 3)`, as shown in the table below. 
...
```{r ANZROD.quintiles.table, echo=FALSE}
APD %>% select(ANZROD, ICUOutcome) %>% 
   cut(breaks = quantile(ANZROD),
       probs = seq)
                                           )
```
...
```{r APACHE.categories, fig.margin = TRUE, message = FALSE, warning = FALSE, echo = FALSE}
#retrieves the categories present in the dataset 
levels(as.factor(APD$APACHECategory))
```

```{r Diagnosis_admissions, fig.margin = TRUE, echo = FALSE, message = FALSE, warning=FALSE}
...
ggplot(data = APD, mapping = aes(x = ICU_ADM_DTM,  fill = (APACHEDiagnosis %in% c(101:111, 1202:1213)))) + 
   geom_histogram() +
   labs(y = "Admissions per week",
        x = "Admission date",
        fill = "Cardiovascular admission?") + 
   scale_fill_brewer(palette = "Pastel2")
```

```{r Head_trauma_admissions, fig.margin = TRUE, echo = FALSE, message = FALSE, warning=FALSE}
#plots stacked bar charts for the total and group specific admissions. Use the numbers in the concatenation brackets (the ones inside `c(...)`) to mark the ones you want displayed. 
ggplot(data = APD, mapping = aes(x = ICU_ADM_DTM,  fill = (APACHEDiagnosis %in% c(601)))) + 
   geom_histogram() +
   labs(y = "Admissions per week",
        x = "Admission date",
        fill = "Head trauma admission?") + 
   scale_fill_brewer(palette = "Pastel2")
...
#APDmortality %>% filter(`Observed Mortality` > 0.001)
...
VV_candidates <- APD %>% filter(PF<=150 & Intubated == TRUE & (ICUOutcome=="Died" | ICULOS/24>6) & AgeICU <= 75) %>% 
    filter(across(ImmuneDisease:Leukaemia, ~ .x == FALSE))

VA_candidates <- APD %>% 
    filter(APACHEDiagnosis %in% 101:111 & ICUOutcome== c("Died","OtherAcuteHospitalIcu") & AgeICU <= 75) %>% 
    filter(across(ImmuneDisease:Leukaemia, ~ .x == FALSE))
...
These result in between `r tally(VV_strict)` and `r tally(VV_candidates)` patients for VV ECMO, and between `r tally(VA_strict)` and `r tally(VA_candidates)` patients for VA ECMO per year. 

For the `r count(VV_candidates)` patients who met VV ECMO oxygenation criteria, `r sum(VV_candidates$ICUOutcome=="Died")` or `r round(sum(VV_candidates$ICUOutcome=="Died") / tally(VV_candidates)*100, 1)`% died. An estimated benefit of 15% absolute mortality reduction suggests that at the very least 2 extra Territorians would be alive at each year's end with access to ECMO facilities, and at the most 6. 
